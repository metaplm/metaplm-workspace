...
export default function ProjectsPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        toast("Proje kaydedilemedi", "error");
        return;
      }
      toast(editingId ? "Proje güncellendi" : "Proje oluşturuldu", "success");
      setShowModal(false);
      setEditingId(null);
      load();
    } catch {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 animate-in pt-16 md:pt-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold" style={{ color: "var(--text)" }}>Projeler</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--muted)" }}>Şirket bazında zaman çizelgesi projelerini yönetin</p>
        </div>
      ...
      <div className="glass rounded-xl p-4 flex flex-col md:flex-row items-start md:items-center gap-3">
        <label className="text-xs font-medium" style={{ color: "var(--muted)" }}>Şirkete göre filtrele:</label>
        <select value={filterCompany} onChange={e => setFilterCompany(e.target.value)} className="text-sm min-w-[200px]">
          <option value="">Tüm Şirketler</option>
          {companies.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        <span className="text-xs" style={{ color: "var(--muted)" }}>{filtered.length} proje</span>
      </div>
      ...
      <div className="glass rounded-2xl overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-sm" style={{ color: "var(--muted)" }}>Henüz proje yok. Başlamak için bir proje oluşturun.</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border)" }}>
                <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted)" }}>Proje Adı</th>
                <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted)" }}>Şirket</th>
                <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted)" }}>Tahmini Tarih</th>
                <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted)" }}>Varsayılan Faturalama</th>
                <th className="text-right px-5 py-3 text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted)" }}>İşlemler</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(p => (
                <tr key={p.id} className="glass-hover transition-colors" style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td className="px-5 py-3 font-mono text-xs" style={{ color: "var(--muted)" }}>
                    {p.estimateDate ? new Date(p.estimateDate).toLocaleDateString("tr-TR") : "—"}
                  </td>
                  <td className="px-5 py-3">
                    <span className="text-xs px-2 py-0.5 rounded-full" style={
                      ...
                      {p.defaultBillable ? "Faturalanabilir" : "Faturalanamaz"}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => setDeleteTarget(p)} className="p-1.5 rounded-lg hover:bg-red-500/20 transition-colors text-red-400"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {showModal && (<ModalPortal>
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)" }} onClick={() => setShowModal(false)}>
          <div className="glass rounded-2xl w-full max-w-md p-6 animate-in" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-semibold" style={{ color: "var(--text)" }}>{editingId ? "Projeyi Düzenle" : "Yeni Proje"}</h2>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs font-medium block mb-1" style={{ color: "var(--muted)" }}>Proje Adı *</label>
                <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="örn. STEP & PDF Dönüştürücü" className="text-sm" />
              </div>

              <div>
                <label className="text-xs font-medium block mb-1" style={{ color: "var(--muted)" }}>Tahmini Tarih</label>
              ...
              <div className="flex items-center justify-between glass rounded-lg px-3 py-2.5">
                <span className="text-sm" style={{ color: "var(--text)" }}>Varsayılan Faturalama</span>
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button className="btn-primary flex-1 text-sm" onClick={save} disabled={saving || !form.name || !form.companyId}>{saving ? "Kaydediliyor..." : (editingId ? "Güncelle" : "Oluştur")}</button>
            </div>
          </div>
        </div>
      </ModalPortal>)}
      ...
      {deleteTarget && (<ModalPortal>
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)" }} onClick={() => setDeleteTarget(null)}>
          <div className="glass rounded-2xl w-full max-w-sm p-6 animate-in" onClick={e => e.stopPropagation()}>
            <h2 className="text-base font-semibold mb-2" style={{ color: "var(--text)" }}>Projeyi Sil</h2>
            <p className="text-sm mb-5" style={{ color: "var(--muted)" }}>
              <span style={{ color: "var(--text)" }}>{deleteTarget.name}</span> projesi silinecek. Bu işlem geri alınamaz.
            </p>
